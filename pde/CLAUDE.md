...
- `fdm.py`：对外导出厚度方向显式 FDM 系数和层间温度增量。
...
`training/tbptt.py` 和 `training/static_topology.py` 在每个时间步先调用显式热源，再调用 PD-GCN，随后用 `total_loss` 约束无源输运增量。多层推理通过 `inference/fdm.py` 使用同一厚度方向 FDM 公式。
