...
from .graph_utils import clone_graph_with_temperature, graph_boundary_nodes, graph_temperature, graph_to_device
from pde import apply_dirichlet_boundary


@torch.no_grad()
def rollout(model, graph_init_or_seq: Union[object, Sequence], steps: int, scale_params, *, return_dimensionless: bool = False):
    if int(steps) <= 0:
        raise ValueError(f"steps must be positive, got {steps}.")

    model_device = next(model.parameters()).device
    graphs = _as_graph_sequence(graph_init_or_seq, int(steps), model_device)
    current_temperature = graph_temperature(graphs[0])
    predictions_star = []
